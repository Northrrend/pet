...
statusFrame.Update = function(self, remXP, remBattles,stepXP)
	self.text1:SetText(format("目前你的等级  :  |cff00ff00%d|r \n已获得的经验  :  |cff00ff00%.2f%%|r \n升级需要经验  :  |cff00ff00%d|r \n上一把的经验  :  |cff00ff00%d|r \n升级需要几场  :  |cffff8c00%d|r \n平均时间  :  |cffff8c00%d|r \n剩余时间  :  |cffff8c00%d|r",
		UnitLevel("player"), floor(10000*(UnitXP("player")/UnitXPMax("player"))+0.5)/100, remXP, stepXP, remBattles, avgTime, remTime))
...
--	local nowTime = GetTime()
	...
	count = count +1
	currentTime = GetTime()
	calculusTime = currentTime - lastTime
	avgTime = calculusTime / count
	remTime = remBattles * avgTime / 60
	lastXP = curXP
	statusFrame:Update(remXP, ceil(remBattles), stepXP, avgTime, remTime)
